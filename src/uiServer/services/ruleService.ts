...
export const getAllRules = async () => {
  try {
    ...
    if (syncType === "git") {
      ...
      const files = await getGitRepositoryFiles(config.git.repoUrl, config.git.branch);
      
      // 使用并行处理获取文件内容
      await processFiles(files, res, syncType, config.git.repoUrl, config.git.branch);
